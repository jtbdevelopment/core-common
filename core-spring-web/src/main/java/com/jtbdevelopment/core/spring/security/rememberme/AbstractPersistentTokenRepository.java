 ...
 */
public abstract class AbstractPersistentTokenRepository<ID extends Serializable, T extends AbstractRememberMeToken<ID>> implements
  ...
  protected abstract T newToken(final ID id, final String username, final String series,
  ...
  public void updateToken(final String series, final String tokenValue, final Date lastUsed) {
    ...
    if (token != null) {
      T newToken = newToken(token.getId(), token.getUsername(), token.getSeries(), tokenValue,
          lastUsed);
  ...
  public void setRememberMeTokenRepository(
      AbstractRememberMeTokenRepository<ID, T> rememberMeTokenRepository) {
    this.rememberMeTokenRepository = rememberMeTokenRepository;
  }
